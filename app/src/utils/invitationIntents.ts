...
//
// The counterpart to `invitationLink()` in ./invitation: that builds links, this
// receives them. Until now MeroCalendar only built them — nothing read an inbound
...
export interface CapturedInvitation {
  /** The invitation token, ready for `decodeInvitationObject`. */
  token: string;
...
function storage(): Storage {
  try {
    ...
    const probe = "__mero_calendar_probe__";
 ...
 */
export function primeInvitationCapture(): void {
  ensureController();
}

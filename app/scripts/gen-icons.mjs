// Generates Mero Calendar's icon set into app/public/ with zero dependencies:
//   favicon.ico (16/32/48 PNG-compressed entries), apple-touch-icon.png,
//   icon-192.png, icon-512.png.
// The rasteriser is shared with mero-pixart's app/scripts/gen-icons.mjs; only
// the palette and `mark()` below differ. favicon.svg stays hand-written and is
// the one asset here that is NOT generated; keep the two in sync.
...
// ---- art -------------------------------------------------------------------
// A calendar page: accent header band, two binding rings, and a 3x2 grid of day
// dots with today picked out. Colours are the app's own tokens from
// app/src/styles/theme.css — accent, text, text-faint, ok — so the icon and the
// UI cannot drift apart.

const BG = "#1d2433";        // --text, the darkest token; reads on a light tab
const PAGE = "#ffffff";      // --bg-1
const ACCENT = "#6c8cff";    // --accent
const DOT = "#97a0b0";       // --text-faint
const TODAY = "#43d17a";     // --ok

function mark(c, fit) {
  // Binding rings first, so the page body overlaps their lower halves and they
  // read as passing behind it rather than floating above.
  draw(c, circle(23, 12.5, 3.4), solid(DOT), fit);
  draw(c, circle(41, 12.5, 3.4), solid(DOT), fit);

  // Page body, then the header band squared off along its bottom edge: the band
  // is a rounded rect that would otherwise curve away from the page sides.
  draw(c, rrect(9, 15, 46, 39, 5), solid(PAGE), fit);
  draw(c, rrect(9, 15, 46, 13, 5), solid(ACCENT), fit);
  draw(c, rrect(9, 23, 46, 5, 0), solid(ACCENT), fit);

  // Day dots. Deliberately only 3x2 and generously spaced — a real 7-column
  // month grid turns to mud below 32 px.
  const R = 2.6;
  for (let row = 0; row < 2; row++) {
    for (let col = 0; col < 3; col++) {
      ...
      draw(c, circle(cx, cy, R), solid(isToday ? TODAY : DOT), fit);
    }
  }
}
